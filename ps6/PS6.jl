...
p = Par(gam=0)
g = ((p.α*p.z_bar*p.β)/(1-(1-p.δ)*p.β))^(1/(1-p.α)) # Some constant
p = Par( gam = g)
...
z = exp.(log_z)
# Get solution
@time Mc  = VFI_fixedpoint(T_EGM,Model(n_k=100,θ_k=2.5,n_z=10,z_grid=z,Π=Π))
...
plot(x,y,f, st=:surface,xlabel="Capital",ylabel="Productivity",title="Value Function-EGM: Surface") # Surface plot
savefig("./Figures/surface_vf")
plot(x,y,f, st=:contour,xlabel="Capital",ylabel="Productivity",nlevels=100, width=2, size=[800,480],title="Value Function -EGM: Contour") # Contour plot
